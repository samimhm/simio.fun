...
// Sidebar component
const AdminSidebar = ({ section, setSection, onDashboardClick }) => (
  <>
    {/* Desktop sidebar: vertical, sticky, sub header */}
    <nav
      className="hidden lg:flex flex-col items-center gap-1 p-2 bg-white rounded-2xl shadow sticky left-0 z-30 w-[100px] h-fit transition-colors duration-200"
      style={{ minWidth: 0, position: 'sticky' }}
    >
      {navItems.map(item => (
        <button
          key={item.key}
          className={`flex flex-col items-center px-1 py-1 rounded-lg text-xs transition-colors duration-200 w-full ${section === item.key ? 'bg-purple-100 text-purple-800 font-semibold' : 'hover:bg-gray-100 text-gray-700'}`}
          onClick={() => {
            setSection(item.key);
    ...
    {/* Mobile sidebar: orizontală, plutitoare absolută în container */}
  </>
);
...
const SimioAdmin = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fdf6e3] flex flex-col items-center py-8 px-2 relative">
      <ToastContainer position="top-right" autoClose={3000} hideProgressBar={false} />
      <div className="w-full pt-14 max-w-7xl flex flex-row items-start justify-start gap-4 relative min-h-screen overflow-y-auto">
        <AdminSidebar section={section} setSection={setSection} onDashboardClick={fetchDashboard} />
        <div className="flex-1 w-full">


      {/* Header */}
      {/* <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-2">
        <span className="text-lg font-bold text-purple-700 text-center md:text-center lg:text-center w-full">SIMIO Admin Panel</span>
      </div> */}
          ...
          <div> 
            ...
            {section === 'rounds' && (
              <div className="bg-white rounded-xl shadow p-4 md:p-6" style={{height: 'calc(100vh - 100px)', overflowY: 'scroll'}}>
      ...
      {/* Mobile sticky nav */}
      <nav className="flex lg:hidden sticky bottom-2 mx-2 flex-row items-center justify-between bg-white shadow-lg z-40 px-2 py-1 rounded-xl" style={{ maxWidth: '100vw', minWidth: '360px'  }}  >
        {navItems.map(item => (
          <button
            key={item.key}
            className={`flex flex-col items-center flex-1 px-1 py-1 rounded-lg text-xs transition-colors duration-200 ${section === item.key ? 'bg-purple-100 text-purple-800 font-semibold' : 'hover:bg-gray-100 text-gray-700'}`}
            onClick={() => {
              setSection(item.key);
              if (item.key === 'dashboard' && typeof fetchDashboard === 'function') fetchDashboard();
            }}
            aria-label={item.label}
          >
            {item.icon}
